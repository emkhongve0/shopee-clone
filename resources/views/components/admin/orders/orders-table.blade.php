...
<div class="bg-[#1e293b] border border-slate-800 rounded-2xl shadow-sm overflow-visible" x-data="{
    ...
    getStatusClass(status) {
        ...
        return classes[s] || 'bg-slate-500/10 text-slate-500 border-slate-500/20';
    }
}">
    ...
    <div class="overflow-x-auto custom-scrollbar">
        <table class="w-full text-left border-collapse">
            <thead>
                <tr
                    ...
                    <th class="py-4 px-6 w-12 text-center">
                        {{-- SỬA: Kiểm tra theo paginatedOrders --}}
            ...
            <tbody class="divide-y divide-slate-800">
                {{-- Empty State: SỬA paginatedOrders --}}
                ...
                {{-- Order Rows: SỬA paginatedOrders --}}
                <template x-for="order in paginatedOrders" :key="order.id">
                    <tr class="hover:bg-slate-800/40 transition-all cursor-pointer group"
                        ...
                        {{-- Order ID --}}
                        <td class="py-4 px-4 font-mono text-xs font-bold text-white"
                            x-text="order.order_number || order.orderId"></td>
                        ...
                        <td class="py-4 px-4">
                            <div class="flex items-center gap-3">
                                <div
                                    ...
                                    <template x-if="order.customer && order.customer.avatar">
                                        <img :src="order.customer.avatar" class="w-full h-full object-cover">
                                    </template>
                                    <template x-if="!order.customer || !order.customer.avatar">
                                        <span x-text="order.customer ? order.customer.name.charAt(0) : 'U'"></span>
                                    </template>
                                </div>
                                <div class="min-w-0">
                                    <p class="text-white font-bold text-sm truncate"
                                        x-text="order.customer ? order.customer.name : 'Khách vãng lai'"></p>
                                    <p class="text-slate-500 text-[10px] truncate"
                                        x-text="order.customer ? order.customer.email : ''"></p>
                                </div>
                            </div>
                        </td>
                        ...
                        {{-- Date --}}
                        <td class="py-4 px-4">
                            <div class="text-white text-xs font-bold" x-text="order.order_date || order.orderDate">
                            </div>
                            <div class="text-slate-500 text-[10px] uppercase font-mono"
                                x-text="order.order_time || '10:30 AM'"></div>
                        </td>

                        {{-- Status Badges --}}
                        <td class="py-4 px-4 text-center">
                            <span :class="getStatusClass(order.payment_status || order.paymentStatus)"
                                class="px-2 py-0.5 rounded-md text-[9px] font-black uppercase border tracking-wider"
                                x-text="order.payment_status || order.paymentStatus"></span>
                        </td>
                        <td class="py-4 px-4 text-center">
                            <span :class="getStatusClass(order.shipping_status || order.shippingStatus)"
                                class="px-2 py-0.5 rounded-md text-[9px] font-black uppercase border tracking-wider"
                                x-text="order.shipping_status || order.shippingStatus"></span>
                        </td>
                        <td class="py-4 px-4 text-center">
                            <span :class="getStatusClass(order.status || order.orderStatus)"
                                class="px-2 py-0.5 rounded-md text-[9px] font-black uppercase border tracking-wider"
                                x-text="order.status || order.orderStatus"></span>
                        </td>

                        {{-- Total --}}
                        <td class="py-4 px-4 text-center font-black text-white text-sm"
                            x-text="new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(order.total_amount || order.total)">
                        </td>
                        ...
                        <td class="py-4 px-6 text-right" @click.stop>
                            <div x-data="{ open: false }" class="relative inline-block">
                                ...
                                <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-100"
                                    ...
                                    <button @click="window.location.href = '/admin/orders/' + order.id + '/download'"
                                        ...
                                        <i class="fas fa-download w-4 text-slate-400"></i> Tải hóa đơn
                                    </button>
